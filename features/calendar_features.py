...
def find_next_event(timestamp, events):
    ...
    ts_naive = make_tz_naive(timestamp)

    future_events = [e for e in events if e['time'] > ts_naive]
...
def find_last_event(timestamp, events):
    ...
    past_events = [e for e in events if e['time'] <= ts_naive]
...
def compute_calendar_features(df_timestamps, calendar):
    ...
    for i, ts in enumerate(df_timestamps):
        ...
        # Make timestamp timezone-naive for all operations
        ts_naive = make_tz_naive(ts)
        ...
        if next_event:
            ...
            time_diff = (next_event['time'] - ts_naive).total_seconds() / 3600.0
        ...
        if last_event:
            ...
            time_since = (ts_naive - last_event['time']).total_seconds() / 86400.0
        ...
        density = count_upcoming_events(ts_naive, calendar, days=7)
